    ...
    <script>
        tailwind.config = {
            theme: {
                extend: {
                    colors: {
                        cafe: '#d7ccc8',       // fond doux café
                        slateform: '#f5f5f5',  // fond clair formulaire
                        accent: '#3B82F6',     // bleu principal
                        bordure: '#9e9e9e'     // gris doux pour les bordures
                    }
                }
            }
        }
...
<body class="bg-cafe min-h-screen text-gray-800">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-4">
            ...
            <div class="space-x-6 flex items-center">
                <a href="{{ url('/') }}" class="text-gray-800 hover:text-accent font-medium flex items-center space-x-1">
                    <i class="ti ti-home"></i><span>Accueil</span>
                </a>
                <a href="{{ url('/visiteurs/create') }}" class="text-gray-800 hover:text-accent font-medium flex items-center space-x-1">
                    <i class="ti ti-user-plus"></i><span>Ajouter visiteur</span>
                </a>
                <a href="{{ url('/locataires') }}" class="text-gray-800 hover:text-accent font-medium flex items-center space-x-1">
                    <i class="ti ti-users-group"></i><span>Locataires</span>
                </a>
                <a href="{{ url('/locataires/create') }}" class="text-gray-800 hover:text-accent font-medium flex items-center space-x-1">
                    <i class="ti ti-user-cog"></i><span>Ajouter locataire</span>
                </a>
                <a href="{{ url('/logout') }}" class="text-red-600 hover:text-red-800 font-medium flex items-center space-x-1">
        ...
        <form method="POST" action="{{ route('visiteurs.store') }}" class="space-y-6">
            ...
            <div x-data="{ visiteurs: [{ nom: '', prenom: '', sexe: 'M', date: '', heure_arrivee: '', heure_depart: '', motif: '' }] }">
                <template x-for="(visiteur, index) in visiteurs" :key="index">
                    <div class="grid grid-cols-1 md:grid-cols-7 gap-4 mb-6 p-4 bg-slateform text-gray-800 rounded-lg border border-bordure">
                        <div>
